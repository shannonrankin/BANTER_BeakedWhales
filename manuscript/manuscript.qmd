...
Advances in computing have led to a dramatic increase in the opportunities to use machine learning for bioacoustic classification tasks. In particular, use of deep neural networks show great promise for large datasets on a wide variety of taxa, including birds (cite), mammals (cite), insects (cite), reptiles (cite), fish (cite) and marine mammals (Franier et al. , Roman Ruiz et al 2023). Developing a neural network is non-trivial and has typically required extensive programming experience, but there are more and more tools that allow people to more readily use them (Kahl et al. 2021)\]. Neural networks typically require significant sample sizes, although there are increasingly new tools to enhance data or combined disparate datasets (Padovese et al. 2023, Nanni et al. 2020, Shorten and Khoshgoftaar 2019, Dufourq et al. 2022). \[There is a significant learning curve to understand how to properly apply and test neural network classification methods on a novel dataset\]. For many bioacoustic classification tasks, modification of existing machine learning methods may be sufficient and have a lower barrier to entry. Random Forest based acoustic classifiers have been found to be relatively easy, efficient, and effective approach to bioacoustic classification (Ross and Allen 2014, Lopez-Baucells et al. 2019).
