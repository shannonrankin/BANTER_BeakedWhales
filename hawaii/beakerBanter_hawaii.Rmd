...
3.  Filter out unwanted data: (1) subset and rename species for BANTER
...
hawaii_confuseMatrix <- rfPermute::confusionMatrix(banter_model_RF)
hawaii_confuseMatrix <- kable(hawaii_confuseMatrix, align = "c", digits = c(0,0,0,0,2,2,2))%>%
  kable_classic()%>%
  column_spec(5, border_right = TRUE)%>%
  row_spec(0, bold = TRUE)%>%
  row_spec(5,hline_after = TRUE)%>%
  row_spec(5, bold = TRUE)%>%
  save_kable(paste(modelname, "hawaii_confuseMatrix.png", sep="_"), zoom = 2)
...
hawaii_ici_confuseMatrix <- kable(hawaii_ici_confuseMatrix, align = "c", digits = c(0,0,0,0,2,2,2))%>%
  ...
  row_spec(5,hline_after = TRUE)%>%
  row_spec(5, bold = TRUE)%>%
  save_kable(paste(modelname_ici, "hawaii_ici_confuseMatrix.png", sep="_"), zoom = 2)
...
png(paste(modelname, "hawaii_proximity.png", sep="_"), width = 20, height = 20, units = 'cm', res = 300)
hawaii_proximityPlot <- plotProximity(banter_model_RF)
dev.off()
hawaii_proximityPlot <- plotProximity(banter_model_RF)

png(paste(modelname_ici, "hawaii_proximity.png", sep="_"), width = 20, height = 20, units = 'cm', res = 300)
...
png(paste(modelname, "hawaii_importance.png", sep="_"), width = 30, height = 25, units = 'cm', res = 300)
plotImportance(banter_model_RF, plot.type="heatmap")
dev.off()
hawaii_importance <- plotImportance(banter_model_RF, plot.type="heatmap")

png(paste(modelname_ici, "hawaii_importance.png", sep="_"), width = 30, height = 25, units = 'cm', res = 300)
...
png(paste(modelname, "hawaii_votes.png", sep="_"), width = 20, height = 20, units = 'cm',  res = 300)
plotVotes(banter_model_RF)
dev.off()
hawaii_votes <- plotVotes(banter_model_RF)

png(paste(modelname_ici, "hawaii_votes.png", sep="_"), width = 20, height = 20, units = 'cm',  res = 300)
