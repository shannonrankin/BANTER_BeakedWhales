...
banter_model_ec <- addBanterDetector(banter_model_ec, banter_data$detectors, ntree=1e4, sampsize=2, importance = TRUE)
...
saveRDS(banter_model_ec, 'satlantic_banter_model_ec_t1e4s2_t1e5s1.rds')
```
...
banter_model_alt <- runBanterModel(banter_model_alt, ntree=1e5, sampsize=2)
summary(banter_model_alt)
```

Once a stable model is identified, save model with tree/sampsize info in the filename.
```{r, save banter alt model, eval=freshRun, eval=freshRun}
saveRDS(banter_model_alt, 'satlantic_banter_model_alt_t1e4s4_t1e5s2.rds')
```
...
banter_model_ec <- readRDS(here('satlantic', 'satlantic_banter_model_ec_t1e4s2_t1e5s1.rds'))
banter_model_ici <- readRDS(here('satlantic', 'satlantic_banter_model_ici_t1e4s4_t1e5s1.rds'))
banter_model_alt <- readRDS(here('satlantic', 'satlantic_banter_model_alt_t1e4s4_t1e5s2.rds'))
```
...
dev.off()
satlantic_importance <- plotImportance(banter_model_ec_RF, plot.type="heatmap")
...
dev.off()
alt_satlantic_importance <- plotImportance(banter_model_alt_RF, plot.type="heatmap")
```
