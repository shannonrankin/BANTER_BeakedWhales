...
banter_model <- readRDS('satlantic_banter_model_t1e4s2_t1e5s1.rds')
```

```{r, banter detector model, eval=freshRun}
banter_model <- initBanterModel(banter_data$events)
banter_model <- addBanterDetector(banter_model, banter_data$detectors, ntree=1e4, sampsize=2, importance = TRUE)
```
...
banter_model <- runBanterModel(banter_model, ntree=1e5, sampsize=1)
summary(banter_model)
```
...
saveRDS(banter_model, 'satlantic_banter_model_t1e4s2_t1e5s1.rds')
```
...
banter_model_ici <- readRDS('satlantic_banter_model_ici_t1e4s4_t1e5s1.rds')
```
...
dev.off()
satlantic_importance <- plotImportance(banter_model_RF, plot.type="heatmap")
...
dev.off()
ici_satlantic_importance <- plotImportance(banter_model_ici_RF, plot.type="heatmap")
```
...
dev.off()
satlantic_votes <- plotVotes(banter_model_RF)
...
satlantic_priors <- classPriors(banter_model_RF, NULL)
```
