...
banter_model_alt <- runBanterModel(banter_model_alt, ntree=1e4, sampsize=5)
summary(banter_model_alt)
```

Once a stable model is identified, save model with tree/sampsize info in the filename.
```{r, save banter alt model, eval=freshRun}
saveRDS(banter_model_alt, 'natlantic_banter_model_alt_t1e4s5_t1e4s5.rds')
```
## BANTER Analytics
...
banter_model_alt <- readRDS('natlantic_banter_model_alt_t1e4s5_t1e4s5.rds')
```
...
dev.off()
natlantic_ec_importance <- plotImportance(banter_model_ec_RF, plot.type="heatmap")
...
plotImportance(banter_model_alt_RF, plot.type="heatmap")
dev.off()
alt_natlantic_importance <- plotImportance(banter_model_alt_RF, plot.type="heatmap")
```
...
natlantic_alt_Figure<- image_append(c(confuse, natlantic_ici_Figure), stack=TRUE)
